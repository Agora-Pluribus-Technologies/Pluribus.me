...
async function loadBlogPosts(sourceUrl, pagesJson) {
  ...
  for (const page of pagesJson) {
    try {
      ...
      if (response.ok) {
        ...
        const postData = parsePostMarkdown(markdown, page);
        posts.push(postData);
      }
    } catch (error) {
  ...
  renderPosts(feedContainer, posts);
}
  ...
  posts.forEach(post => {
    ...
    if (post.image || post.embed) {
      ...
      if (post.embed) {
        ...
        // Handle image path - resolve against remote source
        if (post.image.startsWith("http")) {
          img.src = post.image;
        } else {
          img.src = `${sourceBaseUrl}/${post.image}`;
        }
        img.alt = post.title;
